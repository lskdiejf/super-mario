...
/**
 * Maps.js.
 * Contém funções para criar, preencher e usar mapas.
 */

/**
 * Funções básicas do mapa.
 */
...
function resetMaps()
{
    ...
    /**
     * Mapfuncs começa de tal forma que [X][Y] é window.WorldXY, se existir.
     */
    window.mapfuncs = new Array(9);

    /**
     * Para cada [i][j], se window.WorldIJ existir, ele será usado.
     * Caso contrário, será consultado via AJAX posteriormente.
     */
    for (var i = 1, j; i <= 9; ++i)
    {
    ...
    /**
     * Mapas aleatórios são todas funções de janela.
     */
    mapfuncs["Random"] = {
    ...
    /**
     * No momento, não há muitos mapas especiais.
     */
    mapfuncs["Special"] = {
        Blank: BlankMap
    }
    ...
    /**
     * Mapas não encontrados e sons são carregados via AJAX.
     */
    startLoadingMaps();
}

/**
 * Um novo mapa, que contém configurações gerais para o procedimento do programa.
 */
function Map()
{
    ...
    /**
     * Especificado opcionalmente mais tarde.
     */
    this.time = 400;
...
/**
 * Uma área dentro de um mapa, que contém as PreThings.
 */
...
function setAreaSetting(area, setting, sound)
{
     ...
     * Água fixa.
     */
    if (area.fillStyle.indexOf("Underwater") != -1)
    {
        goUnderWater();
    } else
    {
...
/**
 * Um local dentro de uma Área.
 * Especifica a função de entrada e xloc, se necessário.
 */
function Location(area, entry, xloc)
{
    ...
    /**
     * Se a entrada for verdadeira, use entryPlain (início da camada).
     * Se for válido, use-o.
     * Caso contrário, use entryNormal (soltar de cima).
     */
    this.entry = ((entry == true) ? entryPlain : (entry || entryNormal));
}

/**
 * Um suporte para uma coisa a ser colocada em um local.
 * Armazena as coordenadas, o construtor e quaisquer outros argumentos.
 */
function PreThing(xloc, yloc, type)
{
    ...
    /**
     * Passe quaisquer argumentos após o tipo para a Coisa.
     */
    var args = arrayMake(arguments);

    /**
     * Blank Things são basicamente {}, apenas com o nome Thing em devtools.
     */
...
function setMap(one, two)
{
    ...
    /**
     * A menos que esteja tudo bem, fechar o editor.
     */
    if (!window.canedit && window.editing)
    {
        editorClose(true);
    }

    /**
     * Remova coisas aleatórias.
     */
    removeRandomDisplays();

    /**
     * Se arguments[0] for um vetor, é [one, two].
     */
    ...
    var newmap = new Map();
    var func = mapfuncs[newcurrentmap[0]];

    /**
     * Crie o novo mapa usando o mapfunc, certificando-se de que
     * está carregado.
     */
    if (!func)
    {
        log("Essa seção de mapa não existe (ainda ?):", func);
    ...
    if (!func)
    {
        log("Esse mapa não existe (ainda ?):", func);

        return;
    }

    /**
     * Como a função existe, defina e use-a.
     */
    window.map = newmap;
    ...
    /**
     * Defina as variáveis do mapa de volta para 0.
     *
     * newmap.areanum = newmap.curloc = window.playediting = 0;
     */
    newmap.areanum = newmap.curloc = 0;
    window.area = newmap.area = newmap.areas[0];

    /**
     * Salve a pontuação, se necessário.
     */

    if (window.player && player.power)
    {
        storePlayerStats();
    }
    ...
    /**
     * A redefinição real é feita em shiftToLocation.
     */
    shiftToLocation(0);
}
 ...
 * Os transportes aleatórios de canos de mapa são ["Random", "XXXworld", LocationType].
 * LocationType é também 1 (baixo) ou -1 (alto).
 * Para baixo significa que o jogador está se movendo para baixo;
 * Para cima significa que o jogador está subindo.
 */
function setMapRandom(transport)
{
    ...
    {
        transport = ["Random", "Overworld"];
    }

    /**
     * Na verdade, defina o mapa e mude para o local.
     */
    setMap(transport[0], transport[1]);

    /**
     * Grave coisas específicas aleatórias.
     */
    ...
    if (map.randname == "Sky")
    {
        map.exitloc = ["Random", "Overworld", "Down"];
    }
}
...
{
    /**
     * Certifique-se de que esta é a função correta.
     */
    ...
    /**
     * Redefina tudo relacionado ao programa.
     */
    pause();
    resetGameState();
    ...
    /**
     * Defina a área deste local como atual.
     */
    map.areanum = loc.area;
    window.area = map.area = map.areas[map.areanum];

    /**
     * Limpe tudo, crie o mapa e defina as configurações pós-criação.
     */
    ...
    /**
     * Comece desovando e, em seguida, colocando Player.
     */
    spawnMap();
    player = placePlayer();
    ...
    /**
     * Observe que alguns locs serão pausados manualmente depois disso.
     */
    unpause();

    /**
     * Normalmente, isso não fará nada ou será de um cano.
     */
    loc.entry(player, loc.entrything);

    /**
     * Não se esqueça da parte menos complexa de programar isso!
     */
    TimeHandler.addEvent(AudioPlayer.playTheme, 2);

    /**
     * Os textos são verificados periodicamente por motivos de desempenho.
     */
    TimeHandler.addEventInterval(checkTexts, 117, Infinity);
}

/**
 * Para fazer: adicione outras coisas.
 */
function setAreaPreCreation(area)
{
    /**
     * Limpe os recipientes.
     */
    window.events = [];
    ...
    /**
     * Redefina as configurações de spawn e scroll.
     */
...
function setAreaPostCreation()
{
    ...
    /**
     * Redefinir a gravidade e debaixo d'água.
     */
    ...
    /**
     * Se estiver debaixo d'água, coloque as ondas no topo e o
     * evento de bolha da pessoa.
     */
    if (area.underwater)
    {
        /**
         * Os mapas aleatórios têm um bloqueio para impedir
         * que a pessoa nade muito alto.
         */
        area.presolids.push(new PreThing(0, 0, WaterBlock, area.width));

        /**
         * Mapas não aleatórios também têm um sprite de água
         * (os aleatórios o definem).
         */
        if (!map.random)
        {
            area.presolids.push(new PreThing(0, 16, Sprite, "Water", [area.width / 3, 1]));
        }
    }

    /**
     * Classifique tudo em ordem crescente.
     */
    ...
    /**
     * Se a área tiver loops (na verdade apenas castelos), faça isso.
     */
    ...
    {
        /**
         * Caso contrário, dê a ele um ScrollBlocker em area.width
         * se não for um Sky aleatório.
         */
    ...
    /**
     * O fillstyle é a cor de fundo.
     */
    area.fillStyle = getAreaFillStyle(area.setting);
}

/**
 * Dada uma configuração, retorna a cor de fundo.
 */
function getAreaFillStyle(setting)
{
    ...
    if (stringHas(setting, "Underwater"))
    {
        return "#2038ec";
    }
...
/**
 * Move a geração para um local específico #.
 */
...
/**
 * Funções do programa.
 * Os sólidos são gerados um pouco antes dos grafemas.
 */
function spawnMap()
{
    var area = map.area;
    var rightdiff = QuadsKeeper.getOutDifference();
    var screenright = gamescreen.right + rightdiff;
    var quadswidtht2 = QuadsKeeper.getQuadWidth() * 2 + rightdiff;
    var screenrightpq = screenright + quadswidtht2;
    var arr;
    var arrlen;
    var prething;
    var thing;
...
function goToTransport(transport)
{
    ...
    {
        /**
         * Vai para um novo local.
         */
        shiftToLocation(map.locs[transport]);
    }
}
...
function entryRandom(me)
{
    ...
    if (map.startwidth)
    {
        if (!map.nofloor)
        {
            pushPreFloor(0, 0, map.startwidth);
        }
    } else
    {
        map.startwidth = 0;
     ...
     * 17 * 8.
     */
    map.randtype((map.startwidth + 1) * 8);
    ...
    /**
     * Para fazer: lembre-se de definir o texto e a largura
     * do curmap datadisplay.
     */
    switch(map.entrancetype)
    {
        ...
        case "Vine":
            /**
             * Faça aquela coisa de videira.
             */
            locMovePreparations(player);

            TimeHandler.addEvent(function()
            {
                enterCloudWorld(player, true);
            }, 1);
        ...
        default:
            /**
             * Apenas alcançado por Overworld pela primeira vez.
             *
             * if (map.randname == "Overworld")
             * {
             *     addThing(new Thing(Sprite, "Castle", 1), unitsizet16 * -1, (map.floor - 88) * unitsize);
             * }
             */
            break;
    }
...
{
    /**
     * Existem quatro blocos de nuvens à esquerda. A videira sobe até ter
     * quatro blocos acima das nuvens, depois espera 2 segundos. A pessoa
     * sobe pela esquerda até dois quarteirões do topo, depois troca e
     * pula.
     *
     * if (paused)
     * {
    ...
    if (map.random)
    {
        map.exitloc = getAfterSkyTransport();
    }

    var screenbottom = 140 * unitsize;
    var screentop = 72 * unitsize;
...
{
    if (!pipe.transport || !me.resting || me.right + unitsizet2 > pipe.right || me.left - unitsizet2 < pipe.left)
    {
        return;
    }
...
{
    /**
     * Se a pessoa não estiver descansando ou nadando no programa, ele
     * não deve ser autorizado ao tubo. (o descanso pode ter sido limpo
     * neste ponto, então yvel é como ele verifica).
     *
     * if (abs(me.yvel) > unitsized8 || !map.underwater)
     * {
     *     return;
     * }
     */
...
function pushPreThing(type, xloc, yloc, extras, more)
{
    ...
    /**
     * Caso contrário, coloque-o em sólidos ou chars (cenário tem
     * seu próprio pushPre*).
     */
    if (object.solid && !object.spawn_as_char)
    {
        map.area.presolids.push(prething);
    } else
    {
        map.area.precharacters.push(prething);
    }
...
function pushPreBridge(xloc, yloc, length, sides)
{
    ...
    if (sides instanceof Array)
    {
        if (sides[0])
        {
            pushPreThing(Stone, xloc - 8, yloc, 1, 64);
        }

        if (sides[1])
        {
            pushPreThing(Stone, xloc + length * 8, yloc, 1, 64);
        }
    }
}
...
/**
 * settings = [platwidth, offy1, offy2] (offy é a distância do
 * topo até a plataforma).
 */
function pushPreScale(xloc, yloc, width, settings)
{
    var platwidth = settings[0];
    var offx = platwidth * 2;
    var offy1 = settings[1] + 1.5;
    var offy2 = settings[2] + 1.5;
    var me = pushPreThing(Scale, xloc, yloc, width).object;
...
/**
 * mundos dá o cano [X,Y].
 * offset é a distância entre o scrollblocker e a área principal (8 antes do primeiro cano).
 * block é se a tela deve ser impedida de rolar para isso.
 */
function pushPreWarpWorld(xloc, yloc, worlds, offset, block)
{
    ...
    var len = worlds.length;
    var pipe;
    var i;

    warp = pushPreThing(WarpWorld, xloc, yloc + ceilmax).object;

    var title = pushPreText({
        innerText: "BEM-VINDO À WARP ZONE !",
        style: {
            visibility: "hidden"
        }
    }, startx, 58);
...
/**
 * Pode ser chamado em uma função de mapa ou durante gameplay.
 */
...
/**
 * Criação específica de coisas usadas com frequência.
 */

/**
 * O detector tem coisas armazenadas nele, então as funções de animação
 * podem usá-las. Alguns mundos (8-3, por exemplo) têm uma distância
 * incomum da bandeira ao castelo. Para fazer: use FuncColliders
 * típicos em vez de detectores.
 */
function endCastleOutside(xloc, yloc, castlevel, wall, dist)
{
    ...
    if (wall)
    {
        pushPreScenery("CastleWall", xloc + dist + 72, yloc, wall);
    }

    if (castlevel == 0)
    {
        shiftHoriz(detect2, unitsizet8);
    }
...
function endCastleInside(xloc, last, hard)
{
    ...
    var axe = collider.axe = pushPreThing(CastleAxe, xloc + 104, 40).object;
     ...
     * 104 + 16.
     */
    pushPreThing(ScrollBlocker, xloc + 112, ceilmax);
...
function endCastleInsideFinal(xloc, last)
{
    ...
    if (last)
    {
        ...
        text = stopper.text = [
            pushPreText({innerHTML: "OBRIGADO " + window.player.title.toUpperCase() + "!", style: style}, xloc + 160, 66).object,
            pushPreText({innerHTML: "SUA BUSCA ACABOU.<br>APRESENTAMOS A VOCÊ UMA NOVA MISSÃO.", style: style}, xloc + 148, 50).object,
            pushPreText({innerHTML: "APERTE O BOTÃO B<br>PARA SELECIONAR UM MUNDO.", style: style}, xloc + 148, 26).object
        ];
    } else
    {
        /**
        ...
        text = stopper.text = [
            pushPreText({innerHTML: "OBRIGADO " + window.player.title.toUpperCase() + "!", style: style}, xloc + 160, 66).object,
            pushPreText({innerHTML: "MAS NOSSA PRINCESA ESTÁ EM<br>OUTRO CASTELO!", style: style}, xloc + 148, 50).object
        ];
    }
}
...
function pushPreSectionPass(xloc, yloc, width, height, secnum)
{
    ...
    var secnum = map.area.sections.current || 0;
    var section = map.area.sections[secnum];
    ...
    {
        section.colliders = [passer];
    } else
    {
        section.colliders.push(passer);
    }
}
...
function pushPreSectionFail(xloc, yloc, width, height, secnum)
{
    ...
    if (!section.colliders)
    {
        section.colliders = [failer];
    } else
    {
        section.colliders.push(failer);
    }
}
...
function pushPreTree(xloc, yloc, width)
{
    ...
    /**
     * Embora os troncos das árvores posteriores se sobreponham aos anteriores,
     * tudo bem porque a especificação é indistinguível quando colocado
     * corretamente.
     */
    var dtb = DtB(yloc);
...
function pushPrePipe(xloc, yloc, height, pirhana, intoloc, exitloc)
{
    ...
    var pipe = prepipe.object;

    /**
     * var vert = pushPreThing(PipeVertical, xloc, yloc + height - 8, height - 8);
     */

    if (pirhana)
    {
        pipe.pirhana = pushPreThing(Pirhana, xloc + 4, yloc + height + 12).object;
    }
...
function pushPreCastle(xloc, yloc, big)
{
    ...
    if (big)
    {
        pushPreCastleBig(xloc, yloc);
    } else
    {
        pushPreCastleSmall(xloc, yloc);
    }
}

/**
 * Observação: fora por 16 ou mais.
 */
function pushPreCastleBig(xloc, yloc)
{
    ...
    /**
     * Top especificações alternativas.
     */
    for (i = 0; i < 3; ++i)
    {
        for (j = 0; j < 2; ++j)
    ...
    /**
     * Portas alternativas mais altas.
     */
    for (i = 0; i < 2; ++i)
    {
    ...
    /**
     * Especificação da metade mais alta.
     */
    for(i = 0; i < 5; ++i)
    {
        if (i == 2)
        ...
        {
            pushPreScenerySolid("BrickHalf", xloc + 16 + i * 8, yloc + 48);
        }
    }

    /**
     * Grades para as posições da esquerda.
     */
    for (i = 0; i < 2; ++i)
    {
    ...
    /**
     * Grades para as posições da direita.
     */
    for (i = 5; i < 7; ++i)
    {
    ...
    /**
     * Especificações alternativas mais baixas.
     */
    for (i = 0; i < 2; ++i)
    {
        for (j = 0; j < 3; ++j)
    ...
    /**
     * Portas alternativas mais baixas.
     */
    for (i = 0; i < 3; ++i)
    {
    ...
    /**
     * Preenchimento para a posição da esquerda.
     */
    for (i = 0; i < 2; ++i)
    {
        for (j = 0; j < 5; ++j)
...
/**
 * Para fazer: y, u no cenário de obras.
 */
function pushPreCastleSmall(xloc, yloc)
{
    ...
    /**
     * Especificação de base à esquerda.
     */
    for (i = 0; i < 2; ++i)
    {
        /**
...
{
    /**
     * Posições extravagantes são [xloc, yloc, width, height].
     */
...
/**
 * Isto é para especificações.
 * Sprites terão entradas opcionais para quantos vertical/horizontalmente.
 */
function pushPrePattern(name, xloc, yloc, reps)
{
    var xpos = xloc;
    var pattern = Scenery.patterns[name];
    var info;
    var i;
    var j;
    ...
    for (i = 0; i < reps; ++i)
    {
        ...
        {
            /**
             * Obtenha as informações do sprite.
             */
            info = pattern[j];
...
/**
 * Distância do yloc ao botmax.
 * Assume que yloc está na forma dada por mapfuncs - distância do chão.
 */
...
/**
 * World11 é mantido aqui para evitar o carregamento.
 */
    ...
    map.areas = [
        new Area("Overworld", function()
        {
            ...
            var greeter = "";
                ...
                /**
                 * Para fazer: torne isso não tão dependente da fonte.
                 */
                greeter += "        <br><br>";
                ...
                greeter += "</div>";
                greeter += "";
                greeter += "<div id='boo' style='text-align: right; color: #ffcccc; margin-top: -7px; width: 350px; height: 35px;'>&copy; "+ new Date().getFullYear() +"</div>";
                greeter += "<p id='explanation' style='text-align: center;<!--text-shadow: 2px 2px 1px black;--> margin-left: 7px;'>";
                greeter += "    Mover: Teclas [W - A - S - D]";
                greeter += "    <br>";
                greeter += "    Fogos: [Shift - CTRL - F]";
                greeter += "    <br>";
                greeter += "    Pausar/Mute: [P - M] ";
    ...
    map.areas = [
        new Area(map.areatype, function()
        {
            ...
            if (map.randname == "Underwater")
            {
                ...
                /**
                 * Para fazer: faça uma função unificada para adicionar água
                 * e bloqueador, pela largura do bloco.
                 */
...
function WorldRandomOverworld(map)
{
    ...
    {
        /**
         * castelo adicionado por tipo de entrada.
         */
        for (var i = 0; i < 10; ++i)
        {
            if (randTrue())
            {
                pushRandomGroundScenery(i * 8);
            }
        }
    }
 ...
 *
 */
console.log("Esta é uma cópia offline do player de tela cheia, destinada a testes privados.");
console.log("Normalmente, os mapas são carregados rapidamente por meio de solicitações AJAX; para uso off-line,");
console.log("em vez disso, todos eles foram copiados para a parte mais baixa de maps.js.");
console.log("Se você deseja fazer alterações em um mapa, altere ambos maps.js::WorldXY(map) e Maps/WorldXY.js");
console.log("\n");
console.log("Para usar o Super Mario Bros normalmente, exclua esta mensagem de log e todas as funções depois dela em maps.js.");
...
function World41(map)
{
    ...
    map.areas = [
        new Area("Overworld", function()
        {
             ...
             */
            pushPreFuncCollider(1664, zoneDisableLakitu);
...
function World42(map)
{
    ...
    map.areas = [
        ...
        new Area("Underworld", function()
        {
            ...
            /**
             * Vai para o mundo mais alto - bônus.
             */
            pushPreThing(Brick, 512, 64, [Vine, 5]);
            ...
            /**
             * Vai para o segundo submundo.
             */
            pushPrePipe(672, 0, 24, true, 3);
            ...
            /**
             * Saída do segundo submundo.
             */
            pushPrePipe(1048, 0, 16, false, false, 2);
...
function World63(map)
{
    ...
    map.areas = [
        new Area("Overworld Night Alt2", function()
        {
            ...
            /**
             * Deve mover para cima e para baixo.
             */
            pushPreThing(Platform, 224, 0, 4, [moveFloating, 0, 56]);
...
function World74(map)
{
    ...
    map.areas = [
        new Area("Castle", function()
        {
             ...
             * caindo.
             */
            pushPreThing(Platform, 144, 48, 4, moveFalling);
             ...
             * caindo.
             */
            pushPreThing(Platform, 176, 40, 4, moveFalling);
...
function World84(map)
{
    map.locs = [
         ...
         * Embaixo da agua.
         */
        new Location(3, exitPipeVert, 24),

        /**
         * Cano 2.
         */
        new Location(4, exitPipeVert)
    ];
    ...
    map.areas = [
        new Area("Castle", function()
        {
             ...
             * Cano 1.
             */
            pushPrePipe(152, 16, Infinity, true, false, 1);
            ...
            this.sections = {
                ...
                0: function(xloc)
                {
                     ...
                     * De volta ao cano 1.
                     */
                    pushPrePipe(xloc + bstretch * 8, 16, Infinity, true, 1);
                    ...
                    /**
                     * Para fazer: certifique-se de que está correto.
                     */
                    ...
                    /**
                     * Vai para a fase anterior B.
                     */
                    pushPrePipe(xloc + bstretch * 8 + 304, 40, Infinity, true, 2);
        ...
        new Area("Castle", function()
        {
            ...
            this.sections = {
                ...
                0: function(xloc)
                {
                     ...
                     * Cola B.
                     */
                    pushPrePipe(xloc, 16, Infinity, true, false, 2);
                     ...
                     * Volta ao cano 1.
                     */
                    pushPrePipe(xloc + 136, 16, Infinity, true, 1);
                     ...
                     * Passa C.
                     */
                    pushPrePipe(xloc + 296, jumplev1, 24, true, 3);
        ...
        new Area("Castle", function()
        {
            ...
            this.sections = {
                ...
                0: function(xloc)
                {
                     ...
                     * Volta ao cano 1.
                     */
                    pushPrePipe(xloc + 136, 48, Infinity, true, 1);
                    ...
                    /**
                     * Para Subaquático (Área 3).
                     */
                    pushPrePipe(xloc + 264, 40, Infinity, true, 4);
        ...
        new Area("Underwater Castle", function()
        {
            /**
             * Área 3 (subaquática).
             */
